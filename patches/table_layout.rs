...
use crate::model::style::{Alignment, BorderLine, BorderLineType};
...
use super::border_rendering::{
    ...
    render_transparent_borders, merge_border,
};
    ...
    pub(crate) fn layout_table(
        ...
        if table.border_fill_id > 0 {
            ...
            if let Some(tbl_bs) = styles.border_styles.get(tbl_idx) {
                ...
                // 상단 외곽
                let top_border = &borders[2];
                for c in 0..col_count {
                    if top_border.line_type == BorderLineType::None {
                        h_edges[0][c] = Some(*top_border);
                    } else {
                        h_edges[0][c] = Some(match h_edges[0][c] {
                            Some(existing) => merge_border(&existing, top_border),
                            None => *top_border,
                        });
                    }
                }
                ...
                // 하단 외곽
                let bottom_border = &borders[3];
                for c in 0..col_count {
                    if bottom_border.line_type == BorderLineType::None {
                        h_edges[row_count][c] = Some(*bottom_border);
                    } else {
                        h_edges[row_count][c] = Some(match h_edges[row_count][c] {
                            Some(existing) => merge_border(&existing, bottom_border),
                            None => *bottom_border,
                        });
                    }
                }
                ...
                // 좌측 외곽
                let left_border = &borders[0];
                for r in 0..row_count {
                    if left_border.line_type == BorderLineType::None {
                        v_edges[0][r] = Some(*left_border);
                    } else {
                        v_edges[0][r] = Some(match v_edges[0][r] {
                            Some(existing) => merge_border(&existing, left_border),
                            None => *left_border,
                        });
                    }
                }
                ...
                // 우측 외곽
                let right_border = &borders[1];
                for r in 0..row_count {
                    if right_border.line_type == BorderLineType::None {
                        v_edges[col_count][r] = Some(*right_border);
                    } else {
                        v_edges[col_count][r] = Some(match v_edges[col_count][r] {
                            Some(existing) => merge_border(&existing, right_border),
                            None => *right_border,
                        });
                    }
                }
