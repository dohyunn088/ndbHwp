...
export class CellBorderBgDialog extends ModalDialog {
  ...
  protected onConfirm(): void {
    ...
    } else if (borderScope === 'selected' && this.selectedRange) {
      const dims = this.wasm.getTableDimensions(sec, ppi, ci);
      for (let i = 0; i < dims.cellCount; i++) {
        const cellInfo = this.wasm.getCellInfo(sec, ppi, ci, i);
        const inside = cellInfo.row >= this.selectedRange.startRow && cellInfo.row <= this.selectedRange.endRow &&
                       cellInfo.col >= this.selectedRange.startCol && cellInfo.col <= this.selectedRange.endCol;
        if (inside) {
          this.wasm.setCellProperties(sec, ppi, ci, i, newProps as Partial<CellProperties>);
        }
      }
